...
class App extends Component {
  state = {list: initialTracksList, input: ''}

  onDeleteButton = id => {
    const {list} = this.state

    const result = list.filter(eachItem => eachItem.id !== id)
  ...
  render() {
    ...
    const results = list.filter(eachItem =>
      eachItem.name.toLowerCase().includes(input.toLowerCase()),
    )
    const a = results.length === 0
